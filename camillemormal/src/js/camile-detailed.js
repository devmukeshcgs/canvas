...
/**
* AnimationManager - Handles various types of animations (CSS properties, SVG paths, lines)
* with support for easing curves, delays, and callbacks.
*/
R.AnimationManager = class {
    /**
    ...
    constructor(config) {
        ...
        this.animationFrame = new R.RafR(this.animationLoop);
    }
    ...
    initializeState(config) {
        let state = {
            element: R.Select.el(config.el),
            easing: {
                curve: config.e || "linear"
            },
            duration: {
                origin: config.d || 0,
                current: 0
            ...
            callback: config.cb || false,
            precision: config.r || 2,
            progress: 0,
            easedProgress: 0,
            elapsed: 0
        };

        state.elementCount = state.element.length;

        // Set the appropriate update method based on config
        if (R.Has(config, "update")) {
            state.update = (currentState) => {
                config.update(currentState);
            };
        } else if (R.Has(config, "svg")) {
            state.update = this.updateSvg;
        } else if (R.Has(config, "line")) {
            state.update = this.updateLine;
        } else {
            state.update = this.updateProperties;
        }

        const hasProperties = config.p || false;
        const hasSvg = config.svg || false;
        const hasLine = config.line || false;
        let hasRotation = false;

        if (hasProperties) {
            state.properties = {};
            state.propertyIndex = [];
            const propertyKeys = Object.keys(config.p);
            state.propertyCount = propertyKeys.length;

            for (let i = state.propertyCount - 1; i >= 0; i--) {
                const propertyName = propertyKeys[i];
                const isRotation = propertyName.charAt(0) === "r";
                const indexKey = isRotation && hasRotation ? "r2" : propertyName;

                state.properties[i] = {
                    name: propertyName,
                    origin: {
                        start: config.p[propertyName][0],
                        end: config.p[propertyName][1]
                    },
                    current: config.p[propertyName][0],
                    start: config.p[propertyName][0],
                    end: config.p[propertyName][1],
                    unit: config.p[propertyName][2] || "%"
                };

                hasRotation = isRotation;
                state.propertyIndex[indexKey] = i;
            }
        }
        else if (hasSvg) {
            state.svg = {
                ...
                endValue: config.svg.end,
                originalArrays: {},
                arrays: {},
                values: []
            };

            state.svg.startValue = config.svg.start || R.Ga(state.element[0], state.svg.attribute);
            state.svg.currentValue = state.svg.startValue;
            state.svg.originalArrays.start = R.Svg.split(state.svg.startValue);
            state.svg.originalArrays.end = R.Svg.split(state.svg.endValue);
            state.svg.arrays.start = state.svg.originalArrays.start;
            state.svg.arrays.end = state.svg.originalArrays.end;
            state.svg.arrayLength = state.svg.arrays.start.length;
        }
        else if (hasLine) {
            state.line = {
                isDashed: config.line.dashed,
                coefficients: {
                    start: R.Is.def(config.line.start) ? (100 - config.line.start) / 100 : 1,
                    end: R.Is.def(config.line.end) ? (100 - config.line.end) / 100 : 0
                },
                shapeLengths: [],
                origin: {
                    start: [],
            ...
            for (let i = 0; i < state.elementCount; i++) {
                const element = config.line.elWL || state.element[i];
                state.line.shapeLengths[i] = R.Svg.shapeL(element);

                let dashArray;
                if (state.line.isDashed) {
                    const dashValues = state.line.isDashed.split(/[\s,]/);
                    const dashCount = dashValues.length;
                    let totalDashLength = 0;

                    for (let j = 0; j < dashCount; j++) {
                        totalDashLength += parseFloat(dashValues[j]) || 0;
                    }

                    let pattern = "";
                    const repeatCount = Math.ceil(state.line.shapeLengths[i] / totalDashLength);

                    for (let j = 0; j < repeatCount; j++) {
                        pattern += state.line.isDashed + " ";
                    }

                    dashArray = pattern + "0 " + state.line.shapeLengths[i];
                } else {
                    dashArray = state.line.shapeLengths[i];
                }

                state.element[i].style.strokeDasharray = dashArray;
                state.line.origin.start[i] = state.line.coefficients.start * state.line.shapeLengths[i];
                state.line.origin.end[i] = state.line.coefficients.end * state.line.shapeLengths[i];
                state.line.current[i] = state.line.origin.start[i];
                state.line.start[i] = state.line.origin.start[i];
                state.line.end[i] = state.line.origin.end[i];
            }
        }
    ...
    updateState(options) {
        ...
        const direction = R.Has(options, "reverse") ? "start" : "end";

        if (R.Has(this.state, "properties")) {
            for (let i = this.state.propertyCount - 1; i >= 0; i--) {
                this.state.properties[i].end = this.state.properties[i].origin[direction];
                this.state.properties[i].start = this.state.properties[i].current;

                if (R.Has(config, "p") && R.Has(config.p, this.state.properties[i].name)) {
                    if (R.Has(config.p[this.state.properties[i].name], "newEnd")) {
                        this.state.properties[i].end = config.p[this.state.properties[i].name].newEnd;
                    }
                    if (R.Has(config.p[this.state.properties[i].name], "newStart")) {
                        this.state.properties[i].start = config.p[this.state.properties[i].name].newStart;
                    }
                }
            }
        }
        else if (R.Has(this.state, "svg")) {
            this.state.svg.arrays.start = R.Has(config, "svg") && R.Has(config.svg, "start")
                ? config.svg.start
                : R.Svg.split(this.state.svg.currentValue);

            this.state.svg.arrays.end = R.Has(config, "svg") && R.Has(config.svg, "end")
                ? config.svg.end
                : this.state.svg.originalArrays[direction];
        }
        else if (R.Has(this.state, "line")) {
            for (let i = 0; i < this.state.elementCount; i++) {
                this.state.line.start[i] = this.state.line.current[i];
            }

            if (R.Has(config, "line") && R.Has(config.line, "end")) {
                this.state.line.coefficients.end = (100 - config.line.end) / 100;
                for (let i = 0; i < this.state.elementCount; i++) {
                    this.state.line.end[i] = this.state.line.coefficients.end * this.state.line.shapeLengths[i];
                }
            } else {
                for (let i = 0; i < this.state.elementCount; i++) {
                    this.state.line.end[i] = this.state.line.origin[direction][i];
                }
            }
        }

        this.state.duration.current = R.Has(config, "d")
            ? config.d
            : R.R(this.state.duration.origin - this.state.duration.current + this.state.elapsed);

        this.state.easing.curve = config.e || this.state.easing.curve;
        this.state.easing.calc = R.Is.str(this.state.easing.curve)
            ? R.Ease[this.state.easing.curve]
            : R.Ease4(this.state.easing.curve);

        this.state.delay = (R.Has(config, "delay") ? config : this.state).delay;
        this.state.callback = (R.Has(config, "cb") ? config : this.state).callback;
        this.state.progress = this.state.easedProgress = this.state.duration.current === 0 ? 1 : 0;
        this.delay = new R.Delay(this.startAnimation, this.state.delay);
    }
    ...
    animationLoop(time) {
        if (this.state.progress === 1) {
            ...
            this.state.elapsed = R.Clamp(time, 0, this.state.duration.current);
            this.state.progress = R.Clamp(this.state.elapsed / this.state.duration.current, 0, 1);
            this.state.easedProgress = this.state.easing.calc(this.state.progress);
            this.state.update();
        }
    }
    ...
    updateProperties() {
        ...
        const propertyIndex = this.state.propertyIndex;
        let transformString = "";

        // Update all property values
        for (let i = this.state.propertyCount - 1; i >= 0; i--) {
            properties[i].current = this.interpolate(properties[i].start, properties[i].end);
        }

        // Build transform string
        const xTransform = R.Has(propertyIndex, "x")
            ? properties[propertyIndex.x].current + properties[propertyIndex.x].unit
            : 0;

        const yTransform = R.Has(propertyIndex, "y")
            ? properties[propertyIndex.y].current + properties[propertyIndex.y].unit
            : 0;

        transformString = xTransform + yTransform === 0
            ? 0
            : `translate3d(${xTransform},${yTransform},0)`;

        const rotation = R.Has(propertyIndex, "r")
            ? `${properties[propertyIndex.r].name}(${properties[propertyIndex.r].current}deg)`
            : 0;

        const rotation2 = R.Has(propertyIndex, "r2")
            ? `${properties[propertyIndex.r2].name}(${properties[propertyIndex.r2].current}deg)`
            : 0;

        const scale = R.Has(propertyIndex, "s")
            ? `${properties[propertyIndex.s].name}(${properties[propertyIndex.s].current})`
            : 0;

        const fullTransform = transformString + rotation + rotation2 + scale === 0
            ? 0
            : [transformString, rotation, rotation2, scale].filter(x => x !== 0).join(" ");

        const opacity = R.Has(propertyIndex, "o")
            ? properties[propertyIndex.o].current
            : -1;

        // Apply to elements
        for (let i = this.state.elementCount - 1; i >= 0 && !R.Is.und(this.state.element[i]); i--) {
            if (fullTransform !== 0) {
                this.state.element[i].style.transform = fullTransform;
            }
            if (opacity >= 0) {
                this.state.element[i].style.opacity = opacity;
            }
        }
    }
    ...
    updateSvg() {
        ...
        for (let i = 0; i < svg.arrayLength; i++) {
            svg.values[i] = isNaN(svg.arrays.start[i])
                ? svg.arrays.start[i]
                : this.interpolate(svg.arrays.start[i], svg.arrays.end[i]);

            svg.currentTemp += svg.values[i] + " ";
            svg.currentValue = svg.currentTemp.trim();
        }

        for (let i = 0; i < this.state.elementCount && !R.Is.und(this.state.element[i]); i++) {
            this.state.element[i].setAttribute(svg.attribute, svg.currentValue);
        }
    }

    /** Updates line animations */
    updateLine() {
        const line = this.state.line;

        for (let i = 0; i < this.state.elementCount; i++) {
            const elementStyle = this.state.element[i].style;
            line.current[i] = this.interpolate(line.start[i], line.end[i]);
    ...
    interpolate(start, end) {
        return R.R(R.Lerp(start, end, this.state.easedProgress), this.state.precision);
    }
},
    R.TL = class {
        constructor() {
    ...
    ,
    /**
     * Timeline - Manages a sequence of animations with optional delays between them.
     * Allows playing/pausing all animations in the timeline.
     */
    R.Timeline = class {
        constructor() {
            this.animations = []; // Array to store animations
            this.cumulativeDelay = 0; // Tracks the total delay across animations
        }
         ...
         */
        add(animationConfig) {
            // Add any specified delay to the cumulative delay
            this.cumulativeDelay += R.Has(animationConfig, "delay") ? animationConfig.delay : 0;

            // Set the animation's delay to the current cumulative delay
            animationConfig.delay = this.cumulativeDelay;

            // Create and store the new animation
            this.animations.push(new R.AnimationManager(animationConfig));
        }

        /**
         * Plays all animations in the timeline
         * @param {Object} options - Optional play options
         */
        play(options) {
            this.executeOnAll("play", options);
        }

        /** Pauses all animations in the timeline */
        pause() {
            this.executeOnAll("pause");
        }

        /**
         * Executes a method on all animations in the timeline
         * @param {string} methodName - Name of the method to call
         * @param {Object} [params] - Optional parameters to pass
         */
        executeOnAll(methodName, params) {
            const animationCount = this.animations.length;
            const parameters = params || undefined;

            for (let i = 0; i < animationCount; i++) {
                this.animations[i][methodName](parameters);
            }
        }
    }
    ,
